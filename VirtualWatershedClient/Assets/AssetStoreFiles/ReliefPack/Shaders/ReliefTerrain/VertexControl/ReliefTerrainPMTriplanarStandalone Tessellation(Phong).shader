...
//
//
Shader "Relief Pack/ReliefTerrainPMTriplanarStandalone Tessellation(Phong)" {
	//
	...
	//
	Properties {
		_Phong ("Tess. phong smooth strength", Range(0,1)) = 0.7
		_TessSubdivisions ("Tess. subdivisions (close)", Range(1,32)) = 3
		_TessSubdivisionsFar ("Tess. subdivisions (far)", Range(1,32)) = 3
...
//		_TERRAIN_PosSize ("Top planar rect min(xz to XY) & size(xz to ZW) for global maps", Vector) = (0,0,1000,1000)
	...
	SubShader {
		...
		#pragma surface surf CustomBlinnPhong vertex:vert finalcolor:customFog  tessellate:tessEdge tessphong:_Phong fullforwardshadows nolightmap
		// U5 fog handling
