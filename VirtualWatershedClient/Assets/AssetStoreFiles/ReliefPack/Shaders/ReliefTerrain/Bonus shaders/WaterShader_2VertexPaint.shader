...
//
Shader "Relief Pack/WaterShader_2VertexPaint" {
    Properties {
		...
		TERRAIN_FlowingMap ("Flowingmap", 2D) = "gray" {}
	...
	void surf (Input IN, inout SurfaceOutput o) {
        ...
        o.Normal = lerp(UnpackNormal( tex2D(_BumpMap, rayPos.xy)*control.x + tex2D (_BumpMap2, rayPos.xy)*control.y ), float3(0,0,1), max(p*0.7, TERRAIN_WaterOpacity));
